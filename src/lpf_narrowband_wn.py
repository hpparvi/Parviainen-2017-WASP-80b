...
class LPFC(LPF):
    def __init__(self, use_ldtk=False, n_threads=4, test=False):
        ...
        cols = [c for c in df.columns if 'target_nb' in c]
        pbs = [c[-4:] for c in cols]
        npb = len(pbs)

        times  = npb*[df.bjd.values-TZERO]
        fluxes = list(df[cols].values.T)
        fluxes = map(lambda a: a/median(a[:25]), fluxes)
        ...
        for ilc in range(self.nlc):
            self.priors.append(UP(-1.0, 1.0, 'bcn_%i'%ilc)) ##  sbl + ilc -- Baseline constant
            self.priors.append(UP(-2.0, 2.0, 'btl_%i'%ilc)) ##  sbl + ilc -- Linear time trend
            self.priors.append(UP(-1.0, 1.0, 'bec_%i'%ilc)) ##  sbl + ilc -- Extinction coefficient
            self.priors.append(UP(-1.0, 1.0, 'brl_%i'%ilc)) ##  sbl + ilc -- Linear rotator angle trend
    ...
    def set_pv_indices(self, sbl=None, swn=None):
        ...
        self.ibec = [sbl+4*ilc+2 for ilc in range(self.nlc)]
        self.ibrl = [sbl+4*ilc+3 for ilc in range(self.nlc)]
    ...
    def compute_baseline(self, pv):
        bl = pv[self.ibcn][:,newaxis] + pv[self.ibtl][:,newaxis] * self.ctimes[0] + np.exp(pv[self.ibec][:,newaxis] * self.airmass) + pv[self.ibrl][:,newaxis] * self.rotang
        return bl
    ...
    def fit_baseline(self, pvpop):
        def baseline(pv, ilc):
            return pv[0] + pv[1]*self.ctimes[0] + np.exp(pv[2]*self.airmass) + pv[3]*self.rotang

        pvt = pvpop.copy()
        for i in range(self.nlc):
            m = ~self.otmasks[i]
            pv0 = fmin(lambda pv: ((self.fluxes[i][m]-baseline(pv,i)[m])**2).sum(), 
                       [1,0,0,0], disp=False, ftol=1e-9, xtol=1e-9)
            pvt[:,self.ibcn[i]] = normal(pv0[0], 0.001,            size=pvt.shape[0])
            pvt[:,self.ibtl[i]] = normal(pv0[1], 0.10*abs(pv0[1]), size=pvt.shape[0])
            pvt[:,self.ibec[i]] = normal(pv0[2], 0.01*abs(pv0[2]), size=pvt.shape[0])
            pvt[:,self.ibrl[i]] = normal(pv0[3], 0.01*abs(pv0[3]), size=pvt.shape[0])
        return pvt
