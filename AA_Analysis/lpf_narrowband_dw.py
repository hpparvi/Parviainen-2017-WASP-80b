...
class LPFC(LPF):
    ...
    def compute_transit_2(self, pv):
        ...
        z1 = of.z_circular(lpf.times[0], pv[0], pv[1], _a, _i, self.nt) 
        z2 = of.z_circular(lpf.times[self.npb], pv[0], pv[1], _a, _i, self.nt) 
    ...
    def compute_lc_model(self,pv):
        ...
        trm1,trm2 = mean(tr[:self.npb], 0), mean(tr[self.npb:], 0)
        lc = []
        for i in range(0,self.npb):
            lc.append(bl[i]*tr[i]/trm1)
        for i in range(self.npb,2*self.npb):
            lc.append(bl[i]*tr[i]/trm2)
        return lc

    def compute_lc_model_2(self,pv):
        bl1,bl2 = self.compute_baseline_2(pv)
        tr1,tr2 = self.compute_transit_2(pv)
...
#                zip(self.ibla,self.iblb,self.ibld,self.ctimes,self.rotang,self.airmass)]
    ...
    def compute_baseline_2(self, pv):
        bl1 = pv[self.ibcn[:self.npb]][:,newaxis] + pv[self.ibe1[:self.npb]][:,newaxis] * self.ctimes[0]
        bl2 = pv[self.ibcn[self.npb:]][:,newaxis] + pv[self.ibe1[self.npb:]][:,newaxis] * self.ctimes[self.npb]
        return bl1, bl2


    def lnlikelihood_wn_2(self, pv):
    ...
    def lnlikelihood_wn(self, pv):
        fluxes_m = self.compute_lc_model_2(pv)
    ...
    def lnlikelihood_wn_3(self, pv):
        fluxes_m = self.compute_lc_model(pv)
        return (sum([ll_normal_es(fo, fm, wn) for fo,fm,wn in zip(self.fluxes[:self.npb], fluxes_m[:self.npb], pv[self.iwn[:self.npb]])]) +
                sum([ll_normal_es(fo, fm, wn) for fo,fm,wn in zip(self.fluxes[self.npb:], fluxes_m[self.npb:], pv[self.iwn[self.npb:]])]) )
