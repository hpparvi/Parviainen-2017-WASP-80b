...
class LPFC(LPF):
    def __init__(self, use_ldtk=False, constant_k=True, noise='white'):
        ...
        for ilc in range(self.nlc):
            ...
            self.priors.append(UP(-1e-3, 1e-3, 'be1_%i'%ilc)) ##  sbl + ilc -- Linear elevation trend 1
        ...
        for i in range(2):
            self.priors.extend([UP( -9e-4,   9e-4, 'bra_l_%i'%i),        ##  srbl + 0 -- Linear rotator angle trend
                                UP(6.0e-3, 9.5e-3, 'bra_ea_%i'%i),       ##  srbl + 1 -- Bump amplitude
    ...
    def set_pv_indices(self, sbl=None, swn=None):
        ...
        self.ibe1 = [sbl+3*ilc+1 for ilc in range(self.nlc)]
        self.ibe2 = [sbl+3*ilc+2 for ilc in range(self.nlc)]
        
        if hasattr(self, '_srbl'):
            self.ibr1 = [self._srbl + i for i in range(5)]
            self.ibr2 = [self._srbl + 5 + i for i in range(5)]
